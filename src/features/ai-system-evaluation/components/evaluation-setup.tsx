...
export function EvaluationSetup({
  ...
  const [evaluatingSystem, setEvaluatingSystem] = useState<AISystem | null>(null);
  ...
  useEffect(() => {
    const loadAISystem = async () => {
      if (data.aiSystemIds && data.aiSystemIds.length > 0) {
        const storage = new AISystemsStorage();
        const systems = await storage.getAISystems();
        const system = systems.find(s => s.id === data.aiSystemIds![0]);
        if (system) {
