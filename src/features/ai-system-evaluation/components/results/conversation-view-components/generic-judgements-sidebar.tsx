...
function ResponseJudgementCard({
  ...
  // Build list items from answer phrases
  const judgeModelName = 'Answer Phrase'
  const listItems: JudgementListItem[] = hasAnswerPhrases
    ? record.judgeModelAnswerPhrases!.map((answerPhrase: any, idx: number) => ({
        ...
        guardrailName: judgeModelName
      }))
    : []

  return (
    <div className="flex flex-col gap-0 w-full">
      {/* AI Evaluator Judgement Card */}
      <JudgementItemCard
        expandable={hasAnswerPhrases}
        isExpanded={isExpanded}
        onToggle={() => onToggle('judge-model')}
        expandedItems={listItems}
        onItemHover={onBehaviorHover}
        onItemClick={onBehaviorClick}
      ...
      <AnimatePresence initial={false}>
        {(isAnnotationMode || humanJudgement) && (
          <motion.div
            ...
            className="flex flex-col gap-1 px-2 py-2 border border-gray-200 border-t-0 rounded-b-lg bg-gray-0 overflow-hidden"
          >
            <JudgementDisplay
              variant="human"
...
function GuardrailDetailCard({
  ...
  // Build list items from violations
  const listItems: JudgementListItem[] = (() => {
  ...
  return (
    <JudgementItemCard
      icon={getStatusIcon(detail.judgement)}
      expandable={hasViolations}
      isExpanded={isExpanded}
      onToggle={() => onToggle(uniqueKey)}
      expandedItems={listItems}
      onItemHover={onBehaviorHover}
      onItemClick={onBehaviorClick}
      hoveredBehavior={hoveredBehavior}
            ...
            {hasViolations && (
              <>
                ...
                <span className="font-400">
                  {totalBehaviors} Violation{totalBehaviors > 1 ? 's' : ''}
                </span>
              </>
            )}
