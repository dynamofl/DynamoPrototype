-- Update compliance metrics structure to match jailbreak pattern
-- Migration: 20250123000009_compliance_nested_metrics
-- Purpose: Separate AI-only metrics from AI+guardrails metrics for compliance tests

-- Drop and recreate the function with new nested structure
...
BEGIN
  ...
  -- Build AI system metrics (currently same as combined until guardrails are implemented)
...
BEGIN
  ...
  WHERE status = 'completed'
    AND (config->>'testType' = 'compliance' OR evaluation_type = 'compliance')
    AND metrics ? 'ai_system'; -- Check if new structure exists

  RAISE NOTICE 'Updated % compliance evaluations with nested metrics structure', v_count;
END $$;
...
COMMENT ON FUNCTION calculate_compliance_metrics(UUID) IS
  'Calculates comprehensive compliance metrics from compliance_prompts table.
   Returns JSONB with nested structure matching jailbreak pattern:
   - compliance_rate, violation_rate, unique_topics, unique_policies (top level)
   - ai_system: { tp, tn, fp, fn, accuracy, precision, recall, f1_score }
   - ai_system_with_guardrails: { tp, tn, fp, fn, accuracy, precision, recall, f1_score }
   Currently both nested objects have same values until guardrails are implemented for compliance.';
